...
The SQL Server hosting integration can be used with any application technology, not just C# applications. When you use `WithReference` to reference a SQL Server resource, connection information is automatically injected as environment variables into the referencing application.
